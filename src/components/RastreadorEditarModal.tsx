...
interface RastreadorEditarModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (rastreador: Rastreador) => void
  rastreador: Rastreador | null
}

const RastreadorEditarModal: React.FC<RastreadorEditarModalProps> = ({ isOpen, onClose, onSave, rastreador }) => {
  ...
  // Carregar dados do rastreador quando o modal abrir
  useEffect(() => {
    if (rastreador) {
      setFormData({
        numeroSerial: rastreador.numeroSerial || '',
        imei: rastreador.imei || '',
        status: rastreador.status || 'ativo',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (validateForm() && rastreador) {
      // Converter dados do formulário para o formato esperado
      const rastreadorAtualizado: Rastreador = {
        ...rastreador,
  ...
  if (!isOpen || !rastreador) return null

  return (
    <div className="modal-overlay">
      <div className="modal-container modal-large">
        <div className="modal-header">
          <h2>Editar Equipamento - {rastreador.numeroSerial}</h2>
...
export default RastreadorEditarModal
